...
def cutoff_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if time != 00:
        df = df.iloc[:-1]
    print(f'-> Cutoff Finished.')
    return df


def calculate_profit(df: pd.DataFrame) -> float:
    print(f'-> Calculate Profit Started.')
    book, profit = {}, 0.0
    is_watching = False
    ...
    for index, row in df.iterrows():
        ...
        if bought_price is None and is_watching and all_time_low is not None and row['high'] >= all_time_low * 2:
            ...
            month_diff = relativedelta(index, last_bought_date).years * 12 + relativedelta(index, last_bought_date).months
            if month_diff == 12 or bought_price * 1.5 <= row['close'] or bought_price * 0.8 >= row['close']:
                ...
                bought_price, last_bought_date = None, None
    print(f'-> Calculate Profit Finished.')
    return profit


def at_this_is_the_time(df: pd.DataFrame) -> str:
    print(f'-> Calculate Timing Started.')
    is_watching = False
    all_time_high, all_time_high_date, all_time_low, all_time_low_date = None, None, None, None
    bought_price, last_bought_date = None, None
    for index, row in df.iterrows():
...
def main(include_eth: bool = False, bool_calculate_profit: bool = False) -> None:
    targets = ['BTC']
    if include_eth:
        targets.append('ETH')
